...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        api.auth.me().then((u) => {
          setUserState(u);
          const inLocal = !!localStorage.getItem("absensi_user");
          (inLocal ? localStorage : sessionStorage).setItem("absensi_user", JSON.stringify(u));
        }).catch(() => {});
        return;
      } catch {}
    }
  ...
  return (
    <AuthContext.Provider value={{ user, login, setUser, refreshUser, isLoading, logout }}>
      {children}
